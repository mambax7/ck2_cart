...
//區塊主函式 (熱門商品(ck2_cart_hot))
function ck2_cart_hot($options){
	...
	$sql="select a.specification_sn,sum(a.`amount`) as amount_sum,b.commodity_sn from ".$xoopsDB->prefix("ck2_stores_order_commodity")." as a left join ".$xoopsDB->prefix("ck2_stores_commodity_specification")." as b on a.specification_sn=b.specification_sn group by a.specification_sn order by amount_sum desc";
	$result = $xoopsDB->query($sql) or redirect_header($_SERVER['PHP_SELF'],3, $sql);
	while($all=$xoopsDB->fetchArray($result)){
    foreach($all as $k=>$v){
      $$k=$v;
    }
    $commodity[$commodity_sn]=$commodity_sn;
    $amount_arr[$commodity_sn]+=$amount_sum;
  }

  $in_commodity=implode("','",$commodity);

	$sql = "select a.*,b.filename,d.stores_sn from ".$xoopsDB->prefix("ck2_stores_commodity")." as a left join ".$xoopsDB->prefix("ck2_stores_image_center")." as b on b.col_name='commodity_sn' and b.col_sn=a.commodity_sn left join ".$xoopsDB->prefix("ck2_stores_commodity_kinds")." as c on a.kinds_sn=c.kinds_sn left join ".$xoopsDB->prefix("ck2_stores")." as d on c.stores_sn=d.stores_sn  where a.enable='1' and a.commodity_sn in('$in_commodity')  limit 0,{$options[0]}";
    ...
    if($options[1]=='1'){
		...
		<div style='{$css}font-size:12px;'>
			...
			<div>"._MB_CK2CART_CK2_CART_HOT_AMOUNT."{$amount_arr[$commodity_sn]}</div>
  ...
  arsort($amount_arr);
  foreach($amount_arr as $commodity_sn=>$amount){
    $block.=$com[$commodity_sn];
	}
...
//區塊編輯函式
function ck2_cart_hot_edit($options){
	$chked1_1=($options[1]=="1")?"checked":"";
	$chked1_0=($options[1]=="0")?"checked":"";

	$form="
	"._MB_CK2CART_CK2_CART_HOT_EDIT_BITEM0."
	<INPUT type='text' name='options[0]' value='{$options[0]}'><br>
	"._MB_CK2CART_CK2_CART_HOT_EDIT_BITEM1."
