...
//區塊主函式 (隨機商品(ck2_cart_rand))
function ck2_cart_rand($options){
	...
	$sql = "select a.*,b.filename,d.stores_sn from ".$xoopsDB->prefix("ck2_stores_commodity")." as a left join ".$xoopsDB->prefix("ck2_stores_image_center")." as b on b.col_name='commodity_sn' and b.col_sn=a.commodity_sn left join ".$xoopsDB->prefix("ck2_stores_commodity_kinds")." as c on a.kinds_sn=c.kinds_sn left join ".$xoopsDB->prefix("ck2_stores")." as d on c.stores_sn=d.stores_sn  where a.enable='1' order by rand() limit 0,{$options[0]}";
...
//區塊編輯函式
function ck2_cart_rand_edit($options){
	$chked1_1=($options[1]=="1")?"checked":"";
	$chked1_0=($options[1]=="0")?"checked":"";

	$form="
	"._MB_CK2CART_CK2_CART_RAND_EDIT_BITEM0."
	<INPUT type='text' name='options[0]' value='{$options[0]}'><br>
	"._MB_CK2CART_CK2_CART_RAND_EDIT_BITEM1."
